...
Сайт поддерживает **тёмную и светлую темы**, включая режим "как в системе".

- Управление: иконка-переключатель в шапке (system → light → dark по клику).
- Хранение: `localStorage["nd-theme"]`.
- Без FOUC: тема применяется в `<head>` до первой отрисовки (см. `src/components/theme/theme-script.tsx`).
- Реализация: переопределение CSS-переменных под `:root[data-theme="light"]` в `src/app/globals.css`. Все компоненты ссылаются на токены `--color-*`, поэтому переключение темы происходит мгновенно без перезагрузки.

## Статус

- [x] Фаза 0: инициализация, дизайн-токены, БД-скелет, Better-Auth, envelope encryption, route-скелеты, Docker, CI
- [x] Фаза 1 (часть 1, публичная): каталог + товар + поиск/фильтры + SEO/JSON-LD + sitemap
- [x] Фаза 1.5: заказы + админка + кабинет + чат заказа (polling) + envelope-шифрование чувствительных данных
- [x] Фаза 1.6: светлая тема + 11/13 фирменных обложек + дизайн-апгрейды (aurora, marquee, scroll-reveal, cursor-glow)
- [ ] Фаза 1 (часть 2): Lava.top, Resend, Cloudflare Turnstile, юридические страницы
- [ ] Фаза 2: WebSocket-чат, Telegram-бот, промокоды, блог
